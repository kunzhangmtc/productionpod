...
// 	bBDRLoaded: false,
  ...
  //  console.log(scs);
   ...
   //    this.onIssuesTab();
    //   this.onmousedown(this.onScreenAction());
	...
	// 	setTimeout(this.onTimeout2, 120000);
	...
	/*	var prevProdOrd;
    	var oOperatorsModel = sap.ui.getCore().getModel("Operators");
    	var newProdOrd = oOperatorsModel.getData().ProductionOrder;
    	if(newProdOrd !== prevProdOrd){
  //      	var minutesLater = new Date();
  //          var scs = minutesLater.setMinutes(minutesLater.getMinutes());
  ////  console.log(scs);
            countdowntime = this.checkTaktTime();
    	...
    	var sServiceUrl = this._oComponent.getMetadata().getConfig().serviceConfig.serviceUrl;
		 ...
		 var startTime;
    		var oConfig = {
	                ...
	                var that = this;
	            	var uriTab = "/CheckTaktTimeSet(Werks='9000',Aufnr='"+oOperatorsModel.getData().ProductionOrder+"',Aplfl='"+oOperatorsModel.getData().WorkCentre+"')";
  ...
  //  console.log(scs);*/
   ...
   /* onScreenAction: function(){
    	clearTimeout(timeout);
    	this.onIssuesTab();
    },
    onIssuesTab : function(oEvent){
  //  timeout = 120000;
    var that = this;
...
//	 this.onIssuesTab();
  //   }
    ...
    displayIssMatnr : function(){
	 ...
	 this.oIssueMatnrModel.setSizeLimit(600); /* T20180611.0351 - Increase material OData size limit */
       var oItemSelectTemplate = new sap.ui.core.Item({  
		...
		var oConfig = {
	   	...
	   	var that = this;
	   	var oOperatorsModel = this.getView().getModel("Operators");
		var uri = "IssueMaterialSet?$filter=Arbpl eq '"+oOperatorsModel.getData().WorkCentre+"' and Aufnr eq '"+oOperatorsModel.getData().ProductionOrder+"'";
		//?$filter=ProdCategory eq '" + oEvent.getSource().getSelectedItem().getKey() + "'";
    ...
    onPullCar : function(oEvent){
     var oDialog = new sap.m.Dialog({ contentWidth : "400px"});
	 oDialog.setTitle("Pull Car");	
	 var oManagerName = new sap.m.Label({text: "Swipe your card"});
	 var that = this;
	 var userid;
    ...
    pullCar : function(userid){
	 ...
	 // sap.ui.getCore().getModel("Operators").getData().Operators[listBDR.getSelectedItem().getBindingContext().getProperty("Operator")-1].UName
	  ...
	  var uri = "/PulledCarSet(Pull='0000000001',Aufnr='"+oOperatorsModel.getData().ProductionOrder+"',Aplfl='"+oOperatorsModel.getData().WorkCentre+"')";
	  var that = this;
    ...
    checkPullCar : function(oEvent){
		...
		var userid;
	   	this.oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl, oConfig);
	   	var uri = "IssueAuthSet(Auth='PULL',Category01=' ',Category02=' ',Category03=' ',AuthGroup='"+sap.ui.getCore().byId("idPullCarManager").getValue()+"')";
	   	 this.oDataModel.read(uri, {
        	success : function(oData) {
...
//	  "Pulled" : pulled,
	   ...
	   var oTable=  	this.getView().byId("idIssueLog");
		...
		oTable.setModel(this.oIssueLogModel);
                var that = this;
                   ...
                   cells:[
                    	new sap.m.Text({text : "{Issue}"}),
						new sap.m.Text({text : "{Aplfl}"}),
                    	new sap.m.Text({text : "{Category01}"}),
                    	new sap.m.Text({text : "{Category02}"}),
                    	new sap.m.Text({text : "{Category03}"}),
	            	...
	            	this.oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl, oConfig);
	                var that = this;
	            	var uriTab = "/IssueCaptureSet?$filter=Aufnr eq '" + oOperatorsModel.getData().ProductionOrder + "' and Aplfl eq '"+oOperatorsModel.getData().WorkCentre+"'";
		//?$filter=ProdCategory eq '" + oEvent.getSource().getSelectedItem().getKey() + "'";
	...
	var oDialog = new sap.m.Dialog({ contentWidth : "500px"});
	...
	var that = this;
	 ...
	 var oReslCode = new sap.m.Label({id:"idLResCode",text: "Resolution Code"});
	 var oReslCodeVal = new sap.m.ComboBox({id:"idSResCode", placeholder : "Select Resolution Code", width :"275px",
	 ...
	 var oReslText = new sap.m.Label({id:"idLResText",text: "Resolution Text"});
	 var oReslTextVal = new sap.m.TextArea({id:"idTAResText"});
...
//	  var date = new Date();
	...
	checkMoveCar : function(oEvent){
		...
		var userid;
	   	this.oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl, oConfig);
	   	var uri = "IssueAuthSet(Auth='MOVE_CAR',Category01=' ',Category02=' ',Category03=' ',AuthGroup='"+sap.ui.getCore().byId("idMoveCarManager").getValue()+"')";
	   	 this.oDataModel.read(uri, {
        	success : function(oData) {
        		    if(oData.AuthGroup !== "NOT AUTHORISED"){
        		    var user = oData.AuthGroup;
...
//	  var date = new Date();
	...
	checkOperations : function(oEvent){
        ...
        this.oDataModel.read("GetOperationsForOperator/$count", {
        	urlParameters: "WorkCentreID='" + oOperatorsModel.getData().WorkCentre + "'&OrderNumber='" + oOperatorsModel.getData().ProductionOrder + "'&Operator='" + oEvent.getSource().getId().slice(-1) + "'",
	...
	onRouteMatched: function(oEvent) {
		try{
			...
			jQuery.when(this.oInitialLoadFinishedDeferred).then(jQuery.proxy(function() {
				...
				var sEntityPath = "/ProdOrderOperationCollection(WorkCentreID='"+ oParameters.arguments.workcentre +"',OrderNumber='"+ oParameters.arguments.ordernumber +"',Operator='"+ oParameters.arguments.operator +"',OperationPlanNo='"+ oParameters.arguments.plannumber +"',OperationCounter='"+ oParameters.arguments.operationcounter +"',Operation='"+ oParameters.arguments.operation +"')";
	...
	onSelectBDR : function(oEvent){
        ...
        this._oView.byId("bdrFrame").setContent("<iframe src='" + uri + "#view=FitH' width='100%' height='" + iFrameHeight + "px' frameBorder='0'></iframe>");
	...
	showWorkInstruction : function(){
        ...
        this._oView.byId("workInstructionsFrame").setContent("<iframe src='" + uri + "#view=FitH' width='100%' height='" + iFrameHeight + "px' frameBorder='0'></iframe>");	
	},
	...
	onSignOff : function(oEvent){
		try{
			...
			if(this.checkBDRConfirmation())
			{
				...
				oDataModel.read(uri, {
		        	urlParameters: "Operator='"+selOperator+"'&OperatorName='"+opName+"'&OrderNumber='"+orderNumber+"'&WorkCentreID='"+workCentre+"'",
		        	success : function(oData, oResponse, aErrorResponse){
	...
	onConfirmBDR: function(oEvent){
		try{
			...
			var data = {
		        ...
		        "Confirmedby": sap.ui.getCore().getModel("Operators").getData().Operators[listBDR.getSelectedItem().getBindingContext().getProperty("Operator")-1].UName,
			...
			var uri = "/OperationBDRCollection(WorkCentreID='"+workCentre+"',OrderNumber='"+orderNumber+"',Operator='"+operator+"',OperationPlanNo='"+operatorPlanNo+"',OperationCounter='"+operationCounter+"',Operation='"+operation+"',Docid='"+docId+"',Objtype='"+objectType+"',ObjId='"+objectId+"',DocumentNumber='"+documentNumber+"')";
			oDataModel.update(uri, data, 
			{
				success : function(oData, oResponse, aErrorResponse){
	        		btn.setText("BDR Confirmed");
	...
	checkDataCollectionOperation : function(btn){
		try{
			...
			var uri = "/ProdOrderOperationCollection(WorkCentreID='"+workCentre+"',OrderNumber='"+orderNumber+"',Operator='"+operator+"',OperationPlanNo='"+operationPlanNo+"',OperationCounter='"+operationCounter+"',Operation='"+operation+"')/ToOperationDataCollect";
			oDataModel.read(uri, {
	        	// urlParameters: "Operator='"+selOperator+"'&OrderNumber='"+orderNumber+"'&WorkCentreID='"+workCentre+"'",
	        	success : function(oData, oResponse, aErrorResponse){
	...
	checkDataCollection : function(oEvent){
		...
		/* @whitwo1j:T20180518.0186 */
		/* -start- */
		var order = btn.getParent().getBindingContext().getProperty("OrderNumber");
		/* -end- */
		...
		else 
		{
			...
			else if(validationType == 3)
			{
				...
				oDataModel.read(uri,
				{
					...
					urlParameters: "Material='"+material+"'&Plant='9000'&Serial='"+value+"'&Order='"+order+"'",
					/* -end- */
